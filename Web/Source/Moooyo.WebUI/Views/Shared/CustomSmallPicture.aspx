...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="zxx_in_box">
        <div class="zxx_header">
            ...
            <div>
                <font color="#4fb3d6" size="2">拖拽高亮框可以移动裁剪位置，鼠标移到角柄上可以调整大小。您上传的图片会自动裁剪成两种尺寸</font>
            </div>
        ...
        <div class="zxx_main_con">
        	...
        	<div class="zxx_test_list">
                ...
                <div class="rel">
                    ...
                    <div style="position:absolute; left:490px; top:0px; width:190px; height:40px; color:#9a9a9a; font-size:12px; text-align:center;">
                        <form action="/Shared/CustomSmallPicture" method="post" id="crop_form" class="fl mr10">
                            <input type="hidden" id="x" name="x" />
                            <input type="hidden" id="y" name="y" />
                            <input type="hidden" id="w" name="w" />
                            <input type="hidden" id="h" name="h" />
                        ...
                        <input type="button" id="resetSelectPhoto" value="重选图片" class="fl reg_btn btn" onclick="window.parent.jBox.close(true);" style="display:none; margin:0px; padding:0px;" /><input type="button" id="resetSelectPhoto2" value="重选图片" class="fl reg_btn btn" onclick="window.location.reload();" style="display:none; margin:0px; padding:0px;" />
                    </div>

                    <div style="position:absolute; left:490px; top:60px; width:150px; height:150px; color:#9a9a9a; font-size:12px;">1.(150×150)像素尺寸                   
                    </div>
                    <span id="preview_box" class="crop_preview"><img id="crop_preview" src="" class="hidden" /></span>
                    <div style="position:absolute; left:490px; top:255px; width:150px; height:150px; color:#999999; font-size:12px;">2.(50×50)像素尺寸
                    </div>
                    <span id="preview_box2" class="crop_preview2"><img id="crop_preview2" src="" class="hidden" /></span>
                    <% if (ViewData["memberBindedPlatforms"] != null)
                       { 
                           if(ViewData["memberBindedPlatforms"].ToString() != "") {%>
                    <div class="share-area">
                        <input type="checkbox" id="ifshare" name="ifshare" checked="checked" style="float:left; cursor:pointer;"/>
                        <span style="color:#aaa; float:left; font-family:宋体; font-size:12px; margin-top:1px;">同步到第三方平台</span>
                    </div>
                    <%
                    }
                    } %>
...
<input type="hidden" id="isMemberUploadAvatar" name="isMemberUploadAvatar" value="<%=ViewData["isMemberUploadAvatar"] %>" />
<input type="hidden" id="bindedPlatforms" name="bindedPlatforms" value="<%=ViewData["memberBindedPlatforms"] %>"/>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolderCss" runat="server">
    ...
    <style type="text/css">
        ...
        .crop_preview{position:absolute; left:490px; top:88px; width:150px; height:150px; overflow:hidden; background-color:#dfdfdf;}
        .crop_preview2{position:absolute; left:490px; top:280px; width:50px; height:50px; overflow:hidden; background-color:#dfdfdf;}
        .share-area{position:absolute; left:490px; top:390px; overflow:hidden; text-align:left;}
        .share-area span{width:auto; text-align:left;}
        .share-area input{width:auto; text-align:left;}
        ...
        .zxx_main_con{padding:10px 10px 0px 10px; clear:both; height:390px;}
        .zxx_footer{padding-bottom:0px; border-top:1px solid #dddddd; line-height:26px; padding-top:13px;}
        .zxx_test_list{padding-left:20px; font-size:1.1em; padding-top:5px; line-height:1.3; overflow:hidden; zoom:1; clear:both;}
        ...
        .originalImage{width:450px; height:400px; border:solid 2px #cdcdcd; padding:2px; text-align:center; position:relative;}
...
//                    alert($("#phototype").val())
//                    if($("#phototype").val() == 1) {
//                    alert($("#phototype").val() + "__");
//                        //添加说说内容
//                        var interestId = "";
//                        var permissions = $("#permissions").val();
//                        var suisuiniantype = "7";
//                        var content = "我上传了新的头像。";
//                        var interestIDs = $("#interestIds").val();
//                        var imageIDs = "";
//                        imageIDs = imageIDs == null ? "" : imageIDs;
//                        if (interestIDs == "") {
//                            $.jBox.tip("你没有加粉或创建的兴趣，无法将动态分享到说说", 'error');
//                        }
//                        else {
//                            ContentProvider.InsertMemberContent(permissions, interestIDs, "0", "0", "1", function (data) {
...
//                    }
            ...
            var isMemberUploadAvatar = $("input#isMemberUploadAvatar").val();
            if(isMemberUploadAvatar == "true") {
        ...
        function showCropPanel(photoRelatPath) {
            ...
            $("#crop_preview").attr("src", photoRelatPath + '?' + Math.random(new Date().getTime() / 1000).toString().substring(2, 7));
            $("#crop_preview2").removeClass("hidden").show();
            $("#crop_preview2").attr("src", photoRelatPath + '?' + Math.random(new Date().getTime() / 1000).toString().substring(2, 7));
...
//                        });
        ...
        function shareUploadAvatar() {
            var ifshare = $("#ifshare").attr("checked");
            if (ifshare != null) {
                var bindedPlatforms = $("#bindedPlatforms").val();
                content = "<%=ViewData["memberName"] %>的新头像,传送门";
                var url = "http://www.moooyo.com/Content/TaContent/<%=ViewData["memberId"] %>";
                setTimeout(function () {
