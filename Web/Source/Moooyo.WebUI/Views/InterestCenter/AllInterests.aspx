...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <div id="interestedAuthorCard"></div>
<div class="c976 clearfix">
    ...
    <section class="inter-conbox mt32 fl">
        ...
        <div class="interall mt11 clearfix">
        <h2 class="fl" id="titleTip"><%if ((Request.QueryString["ititle"] == null) || (Request.QueryString["ititle"] == ""))
                                       {%>全部兴趣<span>(<%=Model.allInterestCount%>)</span><% }
                                       else {%>搜索结果<span>(<%=ViewData["searchInterestCount"]%>)</span>&nbsp;&nbsp;<a href="/InterestCenter/AllInterests">取消搜索</a><% } %></h2>
        <div class="fr mt8">
        <form name="frmInterestSerach" method="get" action="/InterestCenter/AllInterests" onsubmit="return interestSerachSubmit()">
        ...
        <ul class="add-myinter-list fl" id="interestContainer">
         <% foreach (var obj in Model.interestListObj)
           { %>
            <li data-interestid="<%=obj.ID %>"><a href="/InterestCenter/InterestFans?iid=<%=obj.ID%>" target="_blank" ><img src="<%= Comm.getImagePath(obj.ICONPath, ImageType.Middle) %>" width="70" height="70" title="<%=obj.Title%>" alt="<%=obj.Title%>" border="0" class="imgWH70" /><br /><%=obj.Title.Length > 5 ? obj.Title.Substring(0, 5) + "<span class=\"letspa--3\">...</span>" : obj.Title%></a>
            </li>
        <% } %>
        </ul>
        </div>
