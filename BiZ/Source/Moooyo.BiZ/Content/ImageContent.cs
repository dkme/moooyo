...
namespace Moooyo.BiZ.Content
{
    ...
    public class ImageContent : PublicContent
    {
        private String content;//图片内容描述
        ...
        public ImageContent() { }
        public ImageContent(String MemberID, ContentPermissions ContentPermissions, List<String> InterestID, String Content, List<Image> ImageList, ImageLayoutType LayOutType, String Type)
        {
            PublicContent obj = new PublicContent(MemberID, ContentPermissions, InterestID, ContentType.Image);
        ...
        public ImageContent(String contentID)
        {
            try
            {
                ImageContent obj = new ImageContent();
                MongoDatabase md = MongoDBHelper.MongoDB;
                MongoCollection<ImageContent> mc = md.GetCollection<ImageContent>(ImageContent.GetCollectionName());
        ...
        /// 图片内容的添加或更新
        /// </summary>
        /// <param name="obj">图片内容对象</param>
        /// <returns>添加或更新操作是否成功</returns>
        public ImageContent Save(ImageContent obj)
        {
            try
            {
                MongoDatabase md = MongoDBHelper.MongoDB;
                MongoCollection<ImageContent> mc = md.GetCollection<ImageContent>(PublicContent.GetCollectionName());
                mc.Save(obj);
                //审核关键字
